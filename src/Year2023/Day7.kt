...
class Hand(
    ...
    private val five = LinkedList<Char>()
    private val four = LinkedList<Char>()
    private val three = LinkedList<Char>()
    private val pairs = LinkedList<Char>()
    private val ones = LinkedList<Char>()

    fun add(pair: Pair<Char, Int>) {
        when (pair.second) {
            5 -> five.add(pair.first)
            4 -> four.add(pair.first)
            3 -> three.add(pair.first)
            2 -> {
                pairs.add(pair.first)
                pairs.sortedWith { char1, char2 ->
                    charToInt(char2).compareTo(charToInt(char1)) // swapped for reversed order
                }
            }
            1 -> {
                ones.add(pair.first)
                ones.sortedWith { char1, char2 ->
                    charToInt(char2).compareTo(charToInt(char1)) // swapped for reversed order
                }
            }
        }
    ...
    override fun compareTo(other: Hand): Int {
        // FIVES
        if (five.size == other.five.size) {
            if (five.isNotEmpty()) {
                return compareCards(other)
            }
        } else {
            return five.size.compareTo(other.five.size)
        }
        // FOURS
        if (four.size == other.four.size) {
            if (four.isNotEmpty()) {
                return compareCards(other)
            }
        } else {
            return four.size.compareTo(other.four.size)
        }
        // THREES OR FULL HOUSE
        if (three.size == other.three.size) {
            if (three.isNotEmpty()) {
                // FH condition
                return if (pairs.size == other.pairs.size) {
                    compareCards(other)
                } else {
                    pairs.size.compareTo(other.pairs.size)
                }
            }
        } else {
            return three.size.compareTo(other.three.size)
        }
        // ONE OR TWO PAIRS
        if (pairs.size != other.pairs.size) {
            return pairs.size.compareTo(other.pairs.size)
        }
        // SAME NUMBER OF PAIRS OR ONES
        return compareCards(other)
    }

    private fun compareCards(other: Hand): Int {
        for (i in 0..4) {
            val result = charToInt(cards[i]).compareTo(charToInt(other.cards[i]))
