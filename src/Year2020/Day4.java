...
public class Day4 {
    ...
    public static void main(String[] args) {
        ...
        for (String pass : array) {
            String[] batches = pass.split(" ");
            HashMap<String, String> fields = new HashMap<>();
            for (String batch : batches) {
                String[] pair = batch.split(":");
                fields.put(pair[0], pair[1]);
            }
            boolean valid = true;
            for (String rule : req) {
                if (fields.get(rule) == null) {
                    valid = false;
                    break;
                }
            }
            if (valid) {
                part1++;
                for (Map.Entry<String, String> values : fields.entrySet()) {
                    switch (values.getKey()) {
                        case "byr":
                            int birthYear = 0;
                            try {
                                birthYear = Integer.valueOf(values.getValue());
                            } catch (Exception e) {
                                valid = false;
                                break;
                            }
                        ...
                        case "iyr":
                            int issueYear = 0;
                            try {
                                issueYear = Integer.valueOf(values.getValue());
                            } catch (Exception e) {
                                valid = false;
                                break;
                            }
                        ...
                        case "eyr":
                            ...
                            try {
                                expirationYear = Integer.valueOf(values.getValue());
                            } catch (Exception e) {
                                valid = false;
                                break;
                            }
                            ...
                            break;
                        case "hgt":
                            String sHeight = values.getValue();
                            int height = 0;
                            try {
                                height = Integer.valueOf(sHeight.substring(0, sHeight.length() - 2));
                            } catch (Exception e) {
                                valid = false;
                                break;
                            }
                            if (!((sHeight.endsWith("cm") && height >= 150 && height <= 193) || (sHeight.endsWith("in") && height >= 59 && height <= 76))) {
                                valid = false;
                                break;
                            }
                            break;
                        case "hcl":
                            String sHairColor = values.getValue();
                            if (sHairColor.length() == 7 && sHairColor.charAt(0) == '#') {
                                for (int i = 1; i < 6; i++) {
                                    char c = sHairColor.charAt(i);
                                    if (!((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f'))) {
                                        valid = false;
                                        break;
                                    }
                            ...
                            break;
                        case "ecl":
                            String sEyeColor = values.getValue();
                            if (!(sEyeColor.equals("amb") ||
                                    sEyeColor.equals("blu") ||
                                    sEyeColor.equals("brn") ||
                                    sEyeColor.equals("gry") ||
                                    sEyeColor.equals("grn") ||
                                    sEyeColor.equals("hzl") ||
                                    sEyeColor.equals("oth"))) {
                                valid = false;
                                break;
                            }
                            break;
                        case "pid":
                            String id = values.getValue();
                            if (id.length() != 9) {
                                valid = false;
                                break;
                            }
