...
/**
 * Created by arenaq on 17.12.2016.
 */
public class Day10 {
    static String in = "bot 49 gives low to bot 118 and high to bot 182\n" +
    ...
    public static class Bot implements Storable {
        ...
        public int removeLower() {
            ...
            if (a.length == 1) {
                ...
                lower = a[0] < a[1] ? a[0] : a[1];
            }
            values.remove(lower);
            return lower;
        }

        public int removeHigher() {
            Integer[] a = values.toArray(new Integer[values.size()]);
            int higher;
            if (a.length == 1) {
                higher = a[0];
            } else {
                higher = a[0] > a[1] ? a[0] : a[1];
            }
            values.remove(higher);
            return higher;
        }
    }

    public static class Output implements Storable {
        Set<Integer> values = new HashSet<>();
    ...
    public static class BotRule {
        int lowToID, highToID;
        boolean lowToBot, highToBot;

        BotRule(int lowToID, int highToID, boolean lowToBot, boolean highToBot) {
            this.lowToID = lowToID;
            this.highToID = highToID;
    ...
    public static void main(String[] args) {
        ...
        Map<Integer, BotRule> botRules = new HashMap<>();

        new BufferedReader(new StringReader(in)).lines().forEach(new Consumer<String>() {
            @Override
            public void accept(String s) {
                boolean lowToBot = false;
                boolean highToBot = false;
                StringTokenizer st = new StringTokenizer(s);
                if ("bot".compareTo(st.nextToken()) == 0) {
                    int botID = Integer.parseInt(st.nextToken());
                    st.nextToken(); // gives
                    st.nextToken(); // low
                    st.nextToken(); // to
                    if ("bot".compareTo(st.nextToken()) == 0) {
                    ...
                    botRules.put(botID, new BotRule(low, high, lowToBot, highToBot));
                } else {
                    boolean valueToBot = false;
                    ...
                    if (valueToBot) {
                        Bot bot = bots.computeIfAbsent(ID, k -> new Bot());
                        bot.put(value);
                    } else {
                        Output output = outputs.computeIfAbsent(ID, k -> new Output());
                        output.put(value);
                    }
                }
            }
        ...
        while (changed) {
            changed = false;
            for (Map.Entry<Integer, BotRule> entry : botRules.entrySet()) {
                Integer ID = entry.getKey();
                BotRule rule = entry.getValue();
                Bot bot = bots.computeIfAbsent(ID, k -> new Bot());
                if (bot.shouldMove()) {
                    int lower = 0;
                    if (rule.lowToBot) {
                        Bot targetForLower = bots.computeIfAbsent((rule.lowToID), k -> new Bot());
                        if (!targetForLower.shouldMove()) {
                            lower = bot.removeLower();
                            targetForLower.put(lower);
                            changed = true;
                        }
                    } else {
                        Output output = outputs.computeIfAbsent(rule.lowToID, k -> new Output());
                        lower = bot.removeLower();
                        output.put(lower);
                        changed = true;
                    }
                    int higher = 0;
                    if (rule.highToBot) {
                        Bot targetForHigher = bots.computeIfAbsent((rule.highToID), k -> new Bot());
                        if (!targetForHigher.shouldMove()) {
                            higher = bot.removeHigher();
                            targetForHigher.put(higher);
                            changed = true;
                        }
                    } else {
                        Output output = outputs.computeIfAbsent(rule.highToID, k -> new Output());
                        higher = bot.removeHigher();
                        output.put(higher);
                        changed = true;
                    }
